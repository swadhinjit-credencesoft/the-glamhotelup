      ...
      <div className="container">
        ...
        <Tabs.Root value={activeTab} onValueChange={setActiveTab} className="bg-white rounded-3xl shadow-xl overflow-hidden border border-gray-100">
          ...
          <Tabs.List className="flex flex-wrap items-center gap-x-4 sm:gap-x-8 gap-y-2 px-6 lg:px-12 pt-8 pb-4 border-b border-gray-100">
            <Tabs.Trigger
              value="rooms"
              className="text-xl sm:text-2xl whitespace-nowrap font-heading font-bold text-gray-400 data-[state=active]:text-adani-blue pb-4 relative group transition-colors"
            >
              Rooms & Suites
              {activeTab === "rooms" && (
                <motion.div layoutId="home-tab-indicator" className="absolute bottom-0 left-0 w-full h-[4px] bg-adani-blue rounded-t-full" />
              )}
            </Tabs.Trigger>
            <Tabs.Trigger
              value="nearby"
                ...
                <div className="w-full lg:w-1/3 p-4 lg:p-6 bg-gray-50/50 flex flex-col gap-3">
                  {ROOMS.map((r) => (
                    <button
                      ...
                      className={`relative flex items-center p-4 lg:p-5 rounded-2xl transition-all text-left group ${activeRoom === r.id ? 'bg-white shadow-lg text-adani-blue border border-gray-100 scale-[1.02]' : 'hover:bg-white/60 text-gray-600'}`}
                    >
                      <div className="w-16 h-12 mr-4 rounded overflow-hidden shrink-0 transition-opacity bg-cover bg-center"
                        style={{ backgroundImage: `url(${r.thumbnail})` }}
                      />

                      <span className="font-bold font-heading text-lg w-full z-10">{r.name}</span>

                      <ChevronRight className={`w-6 h-6 transition-transform opacity-0 -translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 ${activeRoom === r.id ? 'opacity-100 translate-x-0 text-adani-blue' : ''}`} />
                    </button>
                  ))}
                </div>
                    ...
                    >
                      ...
                      <div className="relative z-20 flex flex-col p-8 lg:p-16">
                        <motion.h3
                          ...
                          className="text-4xl lg:text-6xl font-bold font-heading mb-4"
                        >
                          {room.name}
                        </motion.h3>
                        ...
                        <motion.div
                          ...
                          className="flex flex-wrap gap-x-8 gap-y-4 mb-10 border-l-[3px] border-adani-orange pl-6"
                        >
                          {room.highlights.map((h) => (
                            <span key={h.label} className="text-lg lg:text-xl font-semibold flex items-center gap-2">
                              {h.label}
                            </span>
                          ))}
                        </motion.div>
                  ...
                  >
                    ...
                    <div className="relative z-10 p-6 min-h-[180px] flex flex-col justify-end">
                      <div className="flex items-center justify-between mb-3">
                        <span className="text-xs font-bold uppercase tracking-wide px-2 py-1 rounded bg-adani-blue/20 text-white">
                          {place.category}
                        </span>
                        <span className="text-sm font-bold text-adani-orange">{place.distance}</span>
                      </div>
                      <h4 className="font-bold font-heading text-white text-lg leading-tight group-hover:text-adani-orange transition-colors pb-2">
                        {place.name}
                      </h4>
